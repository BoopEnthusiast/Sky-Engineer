	...
	if Input.is_action_pressed(&"select"):
		...
		if is_already_grabbing:
			if player.currently_grabbing is BuildingNode:
				player.currently_grabbing.points[Building.closest_point_to_manipulator] = _move_point_to_point_manipulator(
			...
			elif player.currently_grabbing is ItemShape:
				player.currently_grabbing.item_to_grab.global_position = _move_point_to_point_manipulator(
						player.currently_grabbing.item_to_grab.global_position,
						player.currently_grabbing.grab_weight,
						delta
				)
...
func _move_point_to_point_manipulator(point: Vector3, move_speed: float, delta: float) -> Vector3:
	...
	var lerped_point := point.lerp(player.point_manipulator.global_position, weight)
